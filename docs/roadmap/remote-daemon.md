...
| RD-045 | The install loop guard asks the CONFIG whether quil was installed, not the HOST whether it is there. `alreadyProvisionedFn` is `cfg.RemoteBinary(dest) != ""`, and a recorded path proves an install landed *once* — nothing about now. So any host whose binary disappeared (container rebuilt from an image, home wiped, `~/.local/bin` cleaned, binary moved by an admin, OS reinstalled on the same hostname) dials the recorded path, gets 127, and is told **"Quil was installed on <host>, but will not run there"** with an architecture theory and a `uname -sm` to run — which comes back looking perfectly correct, because the arch was never the problem. The install is never offered, and the only way out is hand-editing `[remote.hosts.<dest>]` out of `config.toml`. **The discriminator already exists and already runs**: `Probe.ExistingPath` is `""` when the host has none, the probe checks absolute paths *before* `command -v` precisely because `~/.local/bin` is invisible to a non-interactive shell, and `runRemoteSetup` calls `RunProbe` fourteen lines into the function the guard is deciding whether to skip — so the fact is fetched one branch too late. Reproduced 2026-08-01 by deleting the binary from the test VM | — | **done** (`healRemoteRecord` replaces `alreadyProvisionedFn`; the guard now switches on `Probe.ExistingPath` and the loop still terminates because "offer forever" needs `ExistingPath == recorded`, which is only true after a successful write. The probe result is handed to `runRemoteSetup` via `setupOptions.probe`, so the attach path costs one ssh round trip rather than two, and `RemedyUpgrade` skips the reconciliation entirely — quil ran over there, so no probe result could change the decision) |
| RD-046 | Self-heal the `[remote.hosts.<dest>] binary` record from what the probe reports, so no user ever edits `config.toml` to recover. Three states, and only the second is today's wrong-arch case: `ExistingPath == ""` ⇒ the record is known-false, **clear it**; `ExistingPath == recorded` ⇒ we ran exactly that path and it failed, keep the wrong-arch message (this is what preserves the loop guard, since "offer forever" needs a path that exists); `ExistingPath != recorded` ⇒ we ran the wrong path, **record the probe's** and re-dial — which also fixes the hand-installed host that was never recorded, a case `offerRemoteInstall`'s own comment already flags as contradicting the probe's summary one line later. **Positive evidence only**: mutate the record when the probe ANSWERED, never on a probe error, timeout or unparsed output. Absence of evidence must not clear it — a record dropped on a transient failure silently downgrades a working host to bare `quil` on the non-interactive PATH, invisible on Debian/Ubuntu, which is the exact failure the record exists to prevent | RD-045 | **done** (`Config.ClearRemoteBinary` plus a shared `mutateConfig` so the record and clear paths cannot drift on the `fs.ErrNotExist` tolerance — a missing config is the normal first-run state, and the clear path runs on the failure branch where a spurious error is least likely to be noticed. All three states are pinned by test, and the two load-bearing bounds were mutation-verified: clearing on a probe error fails `TestHealRemoteRecord_ProbeError_LeavesRecordUntouched`, and disabling the `ExistingPath == recorded` arm fails `TestHealRemoteRecord_ProbeMatchesRecord_RefusesASecondAttempt`) |
