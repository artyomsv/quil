...
- [security/H-repo] The real test-VM host and username in `internal/tui/projectdialog_test.go` and `internal/tui/sidebar.go` of a PUBLIC repo. Replaced with the `artyom@gpu01` placeholder, matching `eefef22`; repo-wide grep clean — round 1. **This entry itself quoted the address it was reporting**, which put it back in the public tree one layer out where a grep over source files did not look; the quotation is removed — round 2
