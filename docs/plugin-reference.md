...
| `discover` | string | No | `""` | When set to `"git"`, replaces the plain directory browser in the setup dialog with a git-repo candidate list: the enclosing repo of the active pane's CWD plus one-level sub-repos, capped at 10, with a final "Browse…" row that falls back to the plain browser. Only meaningful together with `prompts_cwd = true`. Unknown values fail plugin load. |
