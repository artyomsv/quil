...
func TestSyncPaneMeta_SetsWideCanvas(t *testing.T) {
	...
	syncPaneMeta(pane, &PaneInfo{Type: "claude-code"}, true, 0, false, false)
	if !pane.WideCanvas {
		t.Error("syncPaneMeta must set WideCanvas from the passed flag (true)")
	}
	syncPaneMeta(pane, &PaneInfo{Type: "claude-code"}, false, 0, false, false)
	if pane.WideCanvas {
...
func TestSyncPaneMeta_MuteDoesNotDisturbWorking(t *testing.T) {
	...
	pane := NewPaneModel("p1", 1024)
	pane.working = true
	syncPaneMeta(pane, &PaneInfo{Muted: true}, false, 0, false, false)
	if !pane.working {
	...
	pane2.working = true
	syncPaneMeta(pane2, &PaneInfo{Muted: false}, false, 0, false, false)
	if !pane2.working {
