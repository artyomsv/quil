...
**Names and the editor.** Unique ignoring case, trimmed, 1-32 runes (`normalizeGroupName`; the editor stops accepting input at 32). The editor is `Model.groupEdit` — a sibling of the pane-rename status-bar editor, NOT a reuse of `renamingPane` (whose Enter sends `MsgUpdatePane`) — captured in `handleKey` right after `renamingPane`, typed through `msg.Text` + `isPrintableText`, and it STAYS OPEN on `Group name cannot be empty` / `A group with that name already exists`. **Those refusals flash on the LEFT, right after the caret** (`renderStatusBar`): the right side — where every other flash lives — is dropped whole whenever it does not fit beside the left, and with the editor there it needs ~135 columns, so a refusal was invisible at ordinary widths and Enter looked dead. The editor text gives way (its tail, caret included, is kept) so the flash fits, and the left is cut to the bar in every mode, because `.Width` WRAPS an over-wide status line. A `tea.PasteMsg` while it is open goes into it (printable runes, cut at 32), never to the pane behind; and the project, tab and sidebar pane menus refuse to open over it. Names pass `sanitizeRemoteText` at render only: the header, the menu labels (built per open, the raw name riding `ctxMenuItem.groupName`), the status bar.
