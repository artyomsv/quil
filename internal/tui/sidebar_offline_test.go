...
func TestProjectRow_OfflineRendersOrange(t *testing.T) {
	row := projectRow("cluster-management", paneStateCounts{}, 0, glyphLinkParked, false, 22, &OfflineState{Kind: offlineNeedsUpgrade}, rowHighlightNone)
	if !strings.Contains(row, "208") {
		t.Errorf("offline row carries no 208 foreground: %q", row)
	}
	live := projectRow("cluster-management", paneStateCounts{}, 0, "", false, 22, nil, rowHighlightNone)
