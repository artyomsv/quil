...
scrollback_lines = 0            # per-pane scrollback depth; 0 = adaptive to pane count
...
unfocused_dim_enabled = true    # fade the frame while the window is unfocused
unfocused_dim = 0.6             # how far it fades — 0.01 to 0.9
...
| `scrollback_lines` | int | `0` (adaptive) | Lines of scrollback each pane keeps. **Multiplies by pane count** — every pane holds its own terminal emulator whether or not you are looking at it, so a 37-pane workspace at the default measured ~1.1 GB of client memory. Lower it if you run many panes on a memory-tight machine. `0` (unset) selects the **adaptive default**: a workspace-wide budget of 100 000 lines spread across your panes, so ten panes or fewer are unchanged at 10 000 each, 41 panes get about 2 400, and 2 000 is the floor — bounding memory should not make a pane useless. An adapted depth is written to the log once, naming this setting as the override. A value you set yourself always wins and is never adapted; values above `1000000` are clamped, since the setting that exists to *reduce* memory should not be able to exhaust it through a typo. Depth is fixed when a pane is **created** — the terminal emulator cannot release scrollback it has already retained — so a change takes effect on the next launch. Editable without touching this file: **F1 → Settings → Scrollback lines**. |
...
| `unfocused_dim_enabled` | bool | `true` | Whether the frame fades at all while the terminal **window** does not have focus. This is the off switch; `unfocused_dim` below is how far it fades. They are separate keys so that switching the effect off **preserves** your level — turn it back on and you return to the level you chose, not the default. Editable without touching this file: **F1 → Settings → Unfocused dim**, or the command palette (`Alt+Shift+P`, search "dim"). Both apply immediately — no restart. A config file written before this key existed keeps dimming, since an absent key falls back to the default `true`. |
| `unfocused_dim` | float | `0.6` | How far the whole frame fades toward the terminal background while the window is unfocused — panes, tab bar, sidebar, borders and status bar alike. It exists so that typing into a window that only *looks* focused is visible before the first keystroke lands. `0` disables it, which is what a config predating `unfocused_dim_enabled` used to say; values above `0.9` are clamped, because a full blend renders an empty rectangle that is indistinguishable from a crashed TUI, and a negative value (which would blend the wrong way) is treated as `0`. Only the colours change — the text, the layout and every cell width are identical, so nothing shifts when focus comes and goes. Quil asks your terminal for its real default foreground and background (OSC 10/11) and fades toward those, so the result follows your theme rather than assuming black. **The 16 ANSI and 256 palette colours are the exception**: there is no way to query them, so they are resolved against the standard xterm values, and on a heavily-themed terminal a palette colour will shift slightly in hue on blur rather than only fading. A terminal that does not implement focus reporting (DEC 1004) never dims at all, since it never reports losing focus. Editable without touching this file: **F1 → Settings → Unfocused dim level** (any value from `0.01` to `0.9` — the dialog refuses anything it could not display back to you at two decimals, since a level it renders as `0.00` is one you could never re-enter), or the command palette's three presets — subtle `0.30`, normal `0.60`, strong `0.85`. Both apply immediately — no restart. |
