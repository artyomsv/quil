...
**`tabLayoutBusy` = `tabInFlight` + this client's own `pendingSplit` reservation.** `tabInFlight` deliberately did not grow the reservation: it also gates Move to tab…, which has its own rule for reservations (a moved pane never fills one). For an arrangement the reservation is the dangerous case — the new tree is a copy, so `pendingSplit` would be left pointing at a placeholder no tree holds. **Known limit:** other attached TUIs keep their own tree for that tab until the separate layout-sync item lands — existing tabs never adopt the stored layout (`diffLayouts` only sends). Tests: `arrange_test.go`, `arrange_apply_test.go`.
