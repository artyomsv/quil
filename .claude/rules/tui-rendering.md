...
**Placement: beside the largest pane.** `placeArrivingPane` picks the leaf with the largest FRACTION of the tab (product of ratios along its path — a property of the tree alone, so equal trees choose the same leaf whatever each client's window size; ties go to the first leaf, strict `>`). It splits that leaf left|right and falls back to top|bottom only when a left|right half would be under `minPaneW` and a top|bottom half would keep `minPaneH` (`arrivalSplitDir`). The rect comes from the canonical `paneAreaWidth() × (height - chromeHeight)`, not the notes-squeezed width. The only client-dependent input is that fallback, which needs one client's leaf under 20 cells and another's not. **"Equal trees" does not hold for a client with its own worktree reservation** — its tab carries an extra placeholder split (`pendingSplit`) that no other client's copy has, since placeholders are pure client-local runtime state and are never persisted or broadcast, so THAT client's `placeArrivingPane` walk sees a genuinely different tree shape and can pick a different leaf than everyone else; see "A migrated pane never fills a worktree reservation" below for why the divergence is contained rather than a thrash.
...
**A migrated pane never fills a worktree reservation.** The `pendingSplit` fill is skipped when `migrated && worktreeCreates[tab] != ""`: filling it would retire `worktreeCreates`, dispose `worktreeReplaced`, and leave the create's own pane with no leaf. The daemon cannot catch this — the requesting client reserves its placeholder before `create_pane` reaches the daemon — so this guard is the ONLY protection. A bare-root reservation (a replace on a single-pane tab) is kept as the left child of a new split rather than overwritten. An ordinary pending split, which lives for microseconds, is still filled by a migrated pane.

**`adoptMovedPane`** runs on the fallback (largest-leaf) branch that installed a migrated pane, EXCEPT when the target tab is THIS client's own active tab right now: it exits the target's focus mode (so the new split is what the tab shows) and makes the pane the target's `ActivePane`; `finalizeTabPanes` then rewrites the `Active` flags. The user stays in the source tab — the daemon never changes the source project's `ActiveTab` for a pane move, and a dissolved source resolves to the daemon's successor through `indexOfTab`.

**The `tab != m.activeTabModel()` guard exists because every attached client reconciles the same broadcast, and adopting is a claim about what THIS client is looking at, not about the pane that moved.** A second client sitting in the target tab — typing into some other pane there when a THIRD client's move lands — is not the mover and never asked to look at the arriving pane; adopting anyway would steal both `ActivePane` and focus mode out from under it mid-keystroke. The mover's own client never has the target tab active (Enter in the tab picker never switches tabs — `tabpicker.go`), so the guard only ever changes behaviour for a bystander, and it is scoped to the fallback branch alone: the OTHER branch that calls `adoptMovedPane` (filling this client's OWN `pendingSplit` placeholder) is reachable only through this client's own split/replace action in that tab, which is not the bystander case the guard exists for.
