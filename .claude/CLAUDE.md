...
- Settings dialog persistence: every Settings setter (snapshot interval, ghost dimmed, ghost buffer lines, mouse scroll lines, page scroll lines, log level, show disclaimer) flips `m.configChanged = true` so edits survive `cmd/quil/main.go`'s `if m.ConfigChanged()` check on TUI exit. Earlier versions only flagged the disclaimer field, silently dropping every other Settings edit. Log-level changes apply on the next launch (no live re-init); the file handle owned by `main.go` isn't re-plumbed into the Model. The Settings list also exposes a non-config **action row** "Stop daemon" — `settingsField.action` (optional func taking `Model` and returning `(Model, tea.Cmd)`) is invoked on Enter instead of opening the inline editor. The action transitions to `dialogConfirm` with `confirmKind = confirmKindShutdown`; Enter on the confirm sends `MsgShutdown` via the existing IPC client (final snapshot is written in the daemon's stop defers, so panes respawn on next launch) and `tea.Quit`s the TUI in the same `tea.Batch`. Esc on the shutdown confirm returns to the Settings list with the cursor restored to the Stop daemon row, not to dialogNone
