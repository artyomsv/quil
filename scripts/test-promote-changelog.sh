...
# --- 37. a newline-bearing filename is refused ----------------------------
# `grep -Eq` matches per LINE, so a name of "added-a.md<NL>VERSION" would pass
# the name regex on its first line. Both consumers of fragments_of_type re-split
# their output on newline (promote()'s render loop, and write_highlights), so
# the tail would become a bare repo-root-relative path whose bytes get spliced
# into CHANGELOG.md and published as the release body.
#
# Driven through the DIRECTORY GLOB, not --filter-names: that mode reads stdin
# line by line, so such a name arrives as two separate lines and can never reach
# is_fragment_name as one string. Only a real on-disk entry does.
setup newline_name
NL_NAME=$(printf 'added-a.md\nVERSION')
if printf -- '---\nheadline: h\n---\n- **A.**\n' > "$CASE/changelog.d/$NL_NAME" 2>/dev/null &&
   [ -f "$CASE/changelog.d/$NL_NAME" ]; then
  expect_die 'case 37' sh "$PROMOTE" --validate
  assert_changelog 'case 37' "$BASELINE"
  ok '37 a filename containing a newline is refused'
else
  printf '  SKIP 37 (this filesystem does not allow a newline in a filename)\n'
fi

# --- 38. the duplicate-version guard compares fields, not a regex ----------
# In `grep "^V $1 "` the dots are wildcards, so 1.2.3 would match `V 1x2x3`.
setup dup_regex
printf '# generated by scripts/promote-changelog.sh — do not edit\nV 1x1x0 2026-02-03\n' \
  > "$CASE/highlights.txt"
printf -- '---\nheadline: Something\n---\n- **A.**\n' > "$CASE/changelog.d/added-a.md"
sh "$PROMOTE" 1.1.0 2026-02-03 > /dev/null \
  || fail 'case 38: 1.1.0 was refused against an unrelated record V 1x1x0'
grep -q '^V 1.1.0 2026-02-03$' "$CASE/highlights.txt" || fail 'case 38: 1.1.0 not recorded'
ok '38 the duplicate guard matches the version field, not a regex'

# --- 39. a missing highlights file is refused BEFORE any rewrite ----------
# write_highlights runs after `mv "$out" "$CHANGELOG"`, so dying there would
# leave CHANGELOG.md rewritten with the fragments unconsumed — and the retry
# then appends a SECOND `## [1.1.0]` section, breaking the release-note sed
# range for every later extraction.
setup missing_highlights
printf -- '---\nheadline: Something\n---\n- **A.**\n' > "$CASE/changelog.d/added-a.md"
rm -f "$CASE/highlights.txt"
expect_die 'case 39' sh "$PROMOTE" 1.1.0 2026-02-03
assert_changelog 'case 39' "$BASELINE"
[ -e "$CASE/changelog.d/added-a.md" ] || fail 'case 39: fragment was consumed by a refused run'
# The retry, once the file is restored, must produce exactly one section.
printf '# generated by scripts/promote-changelog.sh — do not edit\n' > "$CASE/highlights.txt"
sh "$PROMOTE" 1.1.0 2026-02-03 > /dev/null
[ "$(grep -c '^## \[1\.1\.0\]' "$CHANGELOG_FILE")" -eq 1 ] \
  || fail 'case 39: retry produced a duplicated version section'
ok '39 a missing highlights file is refused before CHANGELOG.md is touched'

# --- 40. a headline on a none-* fragment is refused ------------------------
# Its contents are ignored by design, so a headline written there would
# silently never reach the dialog. README.md states the rule; enforce it.
setup none_headline
printf -- '---\nheadline: Never shown\n---\nnothing user facing\n' \
  > "$CASE/changelog.d/none-thing.md"
expect_die 'case 40' sh "$PROMOTE" --validate
ok '40 a headline on a none-* fragment is refused'

# --- 41. a padded headline is trimmed -------------------------------------
setup headline_padding
printf -- '---\nheadline:   Padded on both sides  \n---\n- **A.**\n' \
  > "$CASE/changelog.d/added-a.md"
sh "$PROMOTE" 1.1.0 2026-02-03 > /dev/null
grep -qx 'A Padded on both sides' "$CASE/highlights.txt" \
  || fail "case 41: headline not trimmed: $(grep '^A ' "$CASE/highlights.txt" | od -c | head -2)"
ok '41 leading and trailing padding is trimmed from a headline'
