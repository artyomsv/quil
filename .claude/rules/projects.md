...
Groups are a CLIENT-side view over `m.projects`: `projectgroups.go` is the pure model (`projectGroups` → `projectGroup{Name, Collapsed, Members}` → `groupMember{Dest, ID}`), its operations and load/save; `projectgroups_model.go` the Model side (save, toggle, prune, `projectSections`); `projectgroups_input.go` the drags, menus, name dialog and keys; `sidebar_hover.go` the hover and drag highlight. Nothing is sent to a daemon, which is what lets one group mix hosts.
...
**Drags.** A header press arms `groupDragging`/`groupDragIdx`; motion reorders groups live (`dragSlot` over the hovered group's whole block, `groupBlockSpanIn`) and sets `groupDragMoved`; the release TOGGLES only when nothing moved — toggling on the press would collapse every group the user starts to drag. A project drag reorders live only inside its own section, by section positions; membership changes on RELEASE (`finishProjectDrag`): on a header → `assign`, on the PROJECTS heading (`sidebarRow.ungroupDrop`, kind still `""`) or an ungrouped project row → `unassign`. The heading is a target because with every project grouped there is no ungrouped row left. **Only a drag that left its press ROW decides membership** (`projectDragMoved`, set by the first motion whose y differs from `projectDragPressY`; sideways jitter on the press row is ignored and tracks nothing). The press itself switches the active project, and a collapsed group above was showing the OLD active project, so its row (two for a remote one) vanishes and every row below moves up — a motionless click, or one that jittered a cell sideways, would otherwise release onto whatever row now sits under the pointer, typically another group's header. A header press TOGGLES only when released on that same header. `TestSidebarGroups_ClickWithoutMotionNeverRegroups`, `…HeaderPressReleasedElsewhereDoesNotToggle`. `sidebarRow.group` is read only behind `inGroup`: every non-group row leaves it 0, group 0's own index. **The dragged project is `projectDragKey` — its `(Dest, ID)` — never the pressed index**: a broadcast can rebuild `m.projects` mid-drag (another client deletes an earlier project), and the index then names the successor, which the release regrouped and persisted. `projectDragIndex` re-resolves it for the reorder, the drop target, the blue row and the release; a project that is gone ends the drag with nothing changed (`TestProjectDrag_*`). The header drag keeps `groupDragIdx`: nothing removes or reorders groups while one is armed — prune removes MEMBERS only, groups load once at start, and every other group mutation (menu, name dialog) clears the drag first.
...
**Persistence.** `project-groups.json` (`config.ProjectGroupsPath()`), loaded in `cmd/quil/main.go` (`LoadProjectGroups` → `SetProjectGroups`, a pure setter — the `SetRecentCWDs` rule). Every change calls `saveGroupsCmd`, which CLONES the groups on the Update goroutine and bumps `groupsSeq`; `groupsWriter` serialises the writes and drops a snapshot older than one already written, because Bubble Tea runs Cmds concurrently with no ordering. That orders ONE process only, so `saveProjectGroups` writes a UNIQUE temp (`os.CreateTemp(dir, "project-groups.json.tmp.*")`, the `persist/notes.go` shape: write, chmod 0600, close, rename, temp removed on any failure) — two TUIs on one machine prune on the same broadcast, and with a fixed `.tmp` one truncated and rewrote it under the other, so a longer write then a shorter one renamed mixed JSON into place for the next start to quarantine (`TestSaveProjectGroups_ConcurrentSavesNeverCorruptTheFile`). Load reads at most 256 KiB + 1 byte; an over-cap, unreadable or unparseable file is renamed to `.bak` — not merely ignored, since the next save would overwrite it — and a symlink is refused and left alone. A failed save logs and flashes `Could not save project groups`; the in-memory state stays. Several TUIs on one machine: last write wins, the other sees it on its next start.

**Pruning only from evidence.** `pruneProjectGroupsFor` runs in the `WorkspaceStateMsg` arm — reached only past `destConnected`, i.e. for a connected daemon whose state arrived — and drops members of THAT dest missing from the daemon's own list, `state.Projects` — NEVER `broadcastProjects(state)`, which synthesises a `proj-interim` placeholder for a tabs-only broadcast from a daemon that does not speak projects, and pruning against that would drop every real member of the dest. Such a broadcast names zero real projects and is skipped. An offline, parked or disconnected host is never pruned (its rows are seeded stand-ins, not the daemon's answer), and a broadcast naming zero projects prunes nothing: a project-capable daemon always has one. A disconnected host's members stay in the file and return with the host. `TestProjectGroups_BroadcastPrunesOnlyItsOwnDestination`.

**Names and the name dialog.** Unique ignoring case, trimmed, 1-32 runes (`normalizeGroupName`; the dialog stops accepting input at 32). New group… and Rename group open `dialogGroupName`, a centred `renderDialog` box (`groupNameDialogWidth`, every line cut to `dialogInnerWidth`), backed by `Model.groupEdit` — `m.dialog` is the open/closed authority. **It was a status-bar editor first and that was the bug**: bottom left, easy enough to miss that New group… read as doing nothing, while it silently held every key and refused every right-click, so right-click read as broken too. Keys arrive through `dispatchDialogKey` (`msg.Text` + `isPrintableText`); it STAYS OPEN on `Group name cannot be empty` / `A group with that name already exists`, shown INSIDE the box (`groupNameRefusal`: the flash `commitGroupEdit` set, while it lasts — `View` draws only the dialog, so the status bar is not on screen) on an error row reserved even when blank, so a refusal does not move the box; an edit clears it. A `tea.PasteMsg` while it is open goes into the name (printable runes, cut at 32), never to the pane behind. Clicks are swallowed by `modalSwallowsMouse`, and the project, tab and group menus also refuse `m.dialog != dialogNone`. Names pass `sanitizeRemoteText` at render only: the header, the menu labels (built per open, the raw name riding `ctxMenuItem.groupName`), the dialog.

**Hover and drag highlight (`sidebar_hover.go`).** The PROJECTS row under a buttonless pointer — a project (both rows of a remote one) or a group header — is painted bg 252 / fg 235; a MOVED project drag (`projectDragMoved`) or header drag (`groupDragMoved`) paints the dragged project's rows, or the header alone, bg 153 / fg 235, and wins over hover. `sidebarHover` is a KEY — `(dest, projectID)` or the group name — never an index, since a broadcast rebuilds the rows. It is resolved through `sidebarRowAt`, the slice the paint uses. The rows take a `rowHighlight` parameter: `renderStyledSegmentsHL` puts the background on every segment and the pad, badges keep their colours, the head's plain text goes dark (`rowHighlight.text`), and the group indent is painted through `rowHighlight.fill` — so a highlighted row is still exactly `w` cells (`TestSidebarGroups_EveryHighlightedRowIsExactlyTheStripWidth`). The drag colour reads the drag flags, so `clearDragState` ends it. **Buttonless motion needs `tea.MouseModeAllMotion`, which `View` now sets whenever the sidebar is painted** (no dialog, `projectSidebarWidth() > 0`) as well as under a context menu. So every pointer move over the whole terminal reaches `Update`: `MouseNone` motion goes to the hover ALONE, ahead of every drag branch — a drag is driven with the button held, and a release lost outside the window must not leave a drag following a pointer nobody presses — and an unchanged key serves the cached frame (`skipRender`, see `tui-rendering.md`). Motion off the PROJECTS rows clears it; under the lazygit overlay (which swallows sidebar clicks) it is cleared too; a context menu's own hover wins while it is open. A pointer leaving the window may send no final motion — the next one corrects it; likewise the highlight can outlive a sidebar toggle, a dialog close or a drag end until the next mouse move (accepted). **The PANES section hovers too.** Any row of a tab's block — heading, pane rows, git rows (`inTab`), not the blank row between blocks — shades the whole block bg 252 (`tabRowHighlight`), keyed `(dest, tabID)`; a pane row, or its git row (`sidebarRow.gitOf`, NOT `paneID`, which every other reader takes to mean the pane's own row), shades that pane's rows bg 249 on top (`paneRowHighlight`), keyed `(dest, tabID, paneID)`. Highlighted, `paneRowHL` splits the state glyph into its own segment so it keeps its colour while the label goes dark. A wheel scroll rebuilds the frame, which drops the `(y, builds)` hover cache, so the next move resolves the row now under the pointer. `sidebar_panes_hover_test.go`.

**The drop target is the release, shown early.** While a MOVED project drag is over somewhere a release would change membership, that row is painted bg 151 / fg 235 with `→ ` (`glyphDropTarget`, U+2192: one cell, no emoji presentation) — another group's header ("join it"), or the PROJECTS heading when the dragged project is grouped and the pointer is on the heading or an ungrouped row ("no group"). `projectDropFor` is the ONE rule: `trackProjectDrag` stores its answer as `Model.projectDrop` (a group NAME or `ungroup`, reset by `clearDragState`, painted only while `projectDragging && projectDragMoved`) and `finishProjectDrag` applies it to the release row, so the green row cannot promise a drop the release will not make. It is resolved from the rows `trackProjectDrag` already built, before any reorder — a reorder only happens over the project's own section, which is never a target. The dragged row stays blue and the hover grey is not painted during any drag (`dragActive`). `TestProjectDrop_*`. A move along the same row reuses the last key while `viewCacheBox.builds` is unchanged (rows change only with a rebuilt frame), so it does not restyle every row; frozen input (link down) skips buttonless frames too. Tests: `projectgroups_highlight_test.go`.

**Menus and keys.** The project menu's *Move to group…* (`ctxActGroupList`) re-populates in place, like Set color…: the groups (`✓` on the current), *New group…*, *No group* (`✓` when ungrouped). The list grows with the user's data, so a box taller than the terminal closes the menu with `Too many groups to list here` rather than silently; group names are cut to the menu box (`renderCtxMenu` also cuts any over-wide label — an uncut one made its pad count negative and panicked). The header menu is the fourth `ctxMenuState` discriminator (`groupName`), closed by the Update prologue when `indexOf` misses: Rename group / Collapse-Expand / Move up / Move down / Delete group — delete ungroups the members and closes nothing. `project.group_toggle` (the active project's group; an ungrouped one is a no-op) and `project.groups_collapse_all` (collapse all, or expand all when all are collapsed) ship unbound, `TierLate`. Tests: `projectgroups_test.go`, `projectgroups_sidebar_test.go`, `projectgroups_input_test.go`.
