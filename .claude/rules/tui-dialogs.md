...
**Esc CANCELS on the step-0 (dialog-CLOSING) branch — it creates nothing.** It used to send a bare `create_tab` there, so that `Ctrl+T` `Esc` stayed the two-keystroke path to the shell tab the key produced before the picker existed. That inverted the meaning Esc has in every other dialog, and the step-0 footer had rendered `Esc cancel` the whole time (`dialog.go`, the render arm) — the screen made a promise the handler broke, which is why this reads as a bug report rather than a preference. The shortcut survives as `Ctrl+T` `Enter` `Enter`, and it is behaviourally IDENTICAL rather than merely similar: `handleCreateTab` substitutes an empty spec for a nil one and both run through the same `resolveRequestedCWD`/`firstPaneType`, so there is no CWD or plugin drift. **That promise is one rename away from being false and is therefore PINNED BY A TEST** (`TestNewTab_EnterEnterCreatesAPlainTerminalTab`): `terminal` is first in `plugin.CategoryOrder()`, but the category holds TWO plugins — `terminal` and `terminal-wide` — ordered by display name, and "Terminal" wins only by being a PREFIX of "Terminal (keeps content on squeeze)". Rename the wide variant to anything sorting earlier, or drop in a user TOML with `category = "terminal"`, and five documents would still promise a plain shell while the keystrokes opened the wide canvas. `createTerminalTab` went with the branch; `sendCreateTab(nil)` is still the wire's "daemon picks", but no TUI path sends one. Esc deeper in is back-navigation and always has been; making every Esc create a tab would remove the only way back a step and make one key mean two things depending on how far in you are. `setupDiscoveryBase` follows the same target split — a split discovers from the active pane's OSC 7 CWD (that pane IS the context), a new tab from the active project's `RootDir`, which is what the daemon's `projectCWD` roots it at anyway. The reachability refusal moved from the keypress into `handleNewTab` (dialog-open time), carrying BOTH carve-outs with it — a nil `m.cur()` and `onlyOfflineProjects()` are the pre-first-broadcast window, not an unreachable host.

**Recent-locations quick pick**: for `prompts_cwd` plugins with no git-discovered repos, `enterSetupOrSplit` offers the last 5 committed CWDs (`Model.recentCWDs`, persisted TUI-owned to `config.RecentCWDsPath(dest)` — `recent-cwds.json` for the empty dest, per-destination otherwise (see `remote-dialogs.md`); the dest comes from `createPaneDialogDest()`, which also decides which machine's plugin availability the dialog shows — via `LoadRecentCWDs`/`SaveRecentCWDs`, pushed in `handleCreatePaneSplit` via pure `pushRecentCWD` — dedup, cap `recentCWDMax=5`, Windows-case-insensitive) as `Model.recentCandidates`, filtered by `existingDirs` (drops deleted folders). This reuses the git repo pick-list UI: `activeCWDPick()` returns `(repoCandidates` else `recentCandidates`, isRecent)`; render + `handleSetupPickKey` (renamed from `handleSetupRepoKey`) are source-agnostic. Enter on a row submits; "Browse…" clears both pick lists and drops to `initSetupBrowser()`. Git discovery keeps priority.
...
**An About row whose sub-dialog is drawn at a width OTHER than `dialogWidth` must return `tea.ClearScreen`.** Both boxes are centred by `lipgloss.Place`, so the narrower one's border sits INSIDE the wider one and the wider one's own edges land on cells Bubble Tea v2's diff considers unchanged — the old border stays painted until something else forces a full frame, which in the report was the user clicking away and back. That is Shortcuts (`shortcutsDialogWidth` 100), Processes (`processesDialogWidth` 92) and What's New (`whatsNewWidth`, up to `whatsNewMaxWidth`); Settings and Plugins render at `dialogWidth` and need nothing. The palette's `palActShortcuts` row and the `system.shortcuts` key already cleared, which is why the same dialog looked correct when reached any other way. `TestAboutMenu_OpeningAWiderDialogClearsTheScreen` is table-driven over the rows and refuses to pass vacuously if a listed dialog's width ever equals `dialogWidth`. The EXIT path needs it too and only What's New already had one (`whatsnew.go`), so the debris was entry-only there.

**The Shortcuts list marks its cursor row** (`> ` replacing the indent, plus `dialogSelected` on the description). `shortcutsCursor` moved and `historyWindow` kept it on screen, but nothing drew it — so the only visible effect of an arrow key was the whole list sliding once the cursor reached the window's edge. The marker REPLACES `shortcutsRowIndent` rather than prefixing it, via `padOrTrunc` and not `truncateToWidth`: that helper cuts but never PADS, so it would hold the key column in line only while `"> "` happens to be exactly the indent width. The cursor is reset on the way IN as well as on Esc — Esc is not the only exit, and any other left a stale cursor for the next open.
