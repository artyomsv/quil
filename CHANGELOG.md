...
**Do not edit this file by hand.** Entries are written one file per pull request under
[`changelog.d/`](changelog.d/README.md); the release workflow collects them into a
version section here and deletes them.
...
### Added
- **Move a pane to another tab.** Right-click a pane and choose *Move to tab…*
  to send it, still running, to any other tab on the same machine. The last
  pane there is split, alternating direction, so panes spiral in instead of
  forming thin columns. You stay where you were, and a tab left empty by the
  move is closed.
- **Tidy a tab's layout.** Right-click a tab and choose *Layout…* to even out
  its panes, or to lay them out as columns, rows, a grid, one main pane with
  the rest stacked beside it, or a spiral. Every pane keeps running. The same
  six are in the command palette, and can be bound to keys in `bindings.toml`
  (`tab.layout_even`, `tab.layout_columns`, `tab.layout_rows`,
  `tab.layout_grid`, `tab.layout_main`, `tab.layout_spiral`).
- **Drag a pane with Alt.** Hold Alt and drag a pane onto the side of another
  pane to put it there, onto the middle to swap the two, or onto a tab in the
  tab bar to move it to that tab. Esc cancels.
...
- **Quil now shows what changed after an upgrade.** Launching a new version opens a
  ...
  collapsed to a count you can expand with `→`. It is also available at any time from
  `F1` → What's New.

  The summary is built from a one-line headline that each pull request now writes
  alongside its changelog entry, so it stays in the register you can read in half a
  minute rather than repeating the full entry.
