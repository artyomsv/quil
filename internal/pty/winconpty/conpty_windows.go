...
func NewWithPipes(inPipeRead, inPipeWrite, outPipeRead, outPipeWrite uintptr, w int, h int, flags int) (c *ConPty, err error) {
	...
	if err := bundledCreatePseudoConsole(c.size, windows.Handle(inPipeRead), windows.Handle(outPipeWrite), uint32(flags), c.hpc); err != nil {
...
func (p *ConPty) Close() error {
	...
	p.closeOnce.Do(func() {
		...
		bundledClosePseudoConsole(*p.hpc)
		err = errors.Join(
...
func (c *ConPty) Resize(w int, h int) error {
	...
	if err := bundledResizePseudoConsole(*c.hpc, size); err != nil {
