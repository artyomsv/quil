...
type PaneModel struct {
	...
	unseen             bool           // work finished while this pane was not focused (a park no longer sets this — see workPark); cleared on focus. CLIENT-owned, with a daemon COPY: every change goes out as MsgUpdatePane{Unseen} (reportUnseen) so it survives a TUI restart, and syncPaneMeta seeds it from the daemon exactly once (unseenSeeded) — never on later broadcasts, which would revert a clear the user just made
	unseenSeeded       bool           // syncPaneMeta has copied the daemon's Unseen once; after that the live value here is the truth
	unseenFromSeed     bool           // the current mark came from that seed, not from a completion this process saw: the previous process already toasted for it, so the on-blur sweep must not toast again on every restart. Cleared the moment this process sets the mark itself
