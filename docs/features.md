...
- **Click a group header** to collapse or expand it. A collapsed group is one row, `▸ name (N)`, carrying the summed badges of its projects — `▲`, the working spinner, `✓`, `◆`, `⌫`, and the link marker when one of its hosts is reconnecting. If the project you are in belongs to a collapsed group, its row still shows under the header.
