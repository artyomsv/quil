...
func TestSyncPaneMeta_PropagatesPending(t *testing.T) {
	...
	defer p.Dispose()
	p.Pending = true
	syncPaneMeta(p, &PaneInfo{ID: "p", Pending: false}, false, 0, false, false)
	if p.Pending {
		t.Error("syncPaneMeta should clear Pending when the daemon reports it spawned")
	}
	syncPaneMeta(p, &PaneInfo{ID: "p", Pending: true}, false, 0, false, false)
	if !p.Pending {
...
func TestSyncPaneMeta_PropagatesSessionAndHistory(t *testing.T) {
	...
	p := NewPaneModel("p", testRingBufSize)
	defer p.Dispose()
	syncPaneMeta(p, &PaneInfo{ID: "p", Type: "claude-code", SessionID: "abc123", HistoryLines: 42}, false, 0, false, false)
