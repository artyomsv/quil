...
**One host holds one project.** A daemon must have at least one tab and a tab must belong to a project, so a host always arrives already holding one — called `Default`, either created when you attach or migrated from tabs that predate projects. Naming a project on such a host **renames that one** rather than adding a second, so whatever was already running there ends up under the name you chose. A host whose project you have already named is refused a second one, and says so; rename it instead. The local daemon is unaffected — it holds as many projects as you like.
