...
- `internal/procscan/` — process-table enumeration + orphan classification for the F1 → Processes dialog. Split like `internal/notify`: the classification is platform-neutral and tested, the `//go:build windows` / `!windows` files hold enumeration syscalls only, so `dev.sh cross` is the Windows half's only gate
